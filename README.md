...
`output.json` is always produced. The committed `output.json` was generated by
the deterministic path; re-run `npm run triage` with a key to produce the
LLM-driven version. Both pass `npm run validate`.
